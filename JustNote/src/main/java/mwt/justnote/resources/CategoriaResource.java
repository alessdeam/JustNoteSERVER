...
public class CategoriaResource {
    ...
    @GET
    @Path("/GetCategorieByIdArea/{token}/{id_area}")
    public Response GetCategorieByIdArea(
            @PathParam("token") String token, @PathParam("id_area") int idArea) {
        try {
            if(authenticationService.checkSession(token)) {
                List<Categoria> categorie = categoriaService.getCategorieByIdArea(idArea);
